...
# 初始化 PID 控制相关变量
integral = np.zeros(Nx)  # 用于积分项
previous_error = np.zeros(Nx)  # 上一时刻的误差
...
for n in range(1, Nt - 1):
    for i in range(2, Nx - 2):
        ...
        # 计算当前误差
        error = -omega_controlled[i, n]  # 目标位移为0，误差为当前位移的负值
        ...
        # 计算 PID 控制项
        control_term = (K_p * error) + (K_i * integral[i]) + (K_d * derivative)

        # 更新位移
        omega_controlled[i, n + 1] = (2 * omega_controlled[i, n] - omega_controlled[i, n - 1] +
                                       dt ** 2 * (-alpha * omega_xx + external_force(i, n) + control_term))
...
ax.set_title(f'Controlled Vibration (K_p={K_p}, K_i={K_i}, K_d={K_d}) over Time and Position')
